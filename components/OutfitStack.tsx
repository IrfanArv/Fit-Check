...
const OutfitStack: React.FC<OutfitStackProps> = ({ outfitHistory, onRemoveLastGarment }) => {
  return (
    <div className="flex flex-col">
      <h2 className="text-xl font-serif tracking-wider text-gray-800 border-b border-gray-400/50 pb-2 mb-3">Tumpukan Baju</h2>
      <div className="space-y-2">
          ...
          >
            ...
            {index > 0 && index === outfitHistory.length - 1 && (
               <button
                ...
                aria-label={`Copot ${layer.garment?.name}`}
              >
        ...
        {outfitHistory.length === 1 && (
            <p className="text-center text-sm text-gray-500 pt-4">Baju yang udah dipake bakal muncul di sini. Coba pilih baju di bawah.</p>
        )}
      </div>
    </div>
  );
